...
const getPlatformId = (browser) => {
    ...
    return platformType ? platformType.toString().charCodeAt(0).toString() : '1'; //chrome
};
...
const generateMessageIDV2 = (userId) => {
    ...
    random.copy(data, 28);
    const hash = (0, crypto_1.createHash)('sha256').update(data).digest();
    return 'ArabDevs_' + hash.toString('hex').toUpperCase().substring(0, 18);
};
exports.generateMessageIDV2 = generateMessageIDV2;
// generate a random ID to attach to a message
const generateMessageID = () => 'ArabDevs_' + (0, crypto_1.randomBytes)(6).toString('hex').toUpperCase();
exports.generateMessageID = generateMessageID;
